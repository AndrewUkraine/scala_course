...
object SelfWork extends App {
  ...
  val sort = nums.filter(_ % 2 == 0).sum
  println(sort)
  val sort2 = nums.filter(_ % 2 != 0).sum
  println(sort2)
  ...
  var factorial: Function1[Int, Int] = new Function1[Int, Int] {
    def apply (x:Int): Int = {
      @tailrec def loop(x: Int, y: Int = 1): Int = {
        if (x<=1) y
        else loop(x-1, x*y)
      }
      loop(x)
  ...
  companies.sortBy(_.capitalization).take(3)
  companies.sum.capitalization
