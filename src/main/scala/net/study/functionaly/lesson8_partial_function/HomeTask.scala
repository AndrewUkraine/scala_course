package net.study.functionaly.lesson8_partial_function
...
object HomeTask extends App {
  ...
  val defaultPaymentDomain: PaymentDomain = {
    case paymentRequest: PaymentRequest => PaymentResult(paymentRequest, PaymentResponse(404, -1))
  }

  def createPaymentDomain(paymentService: PaymentService): PaymentDomain = {
    case paymentRequest: PaymentRequest if paymentService.canProcess(paymentRequest.productType) =>
      val token: (TokenProvider) = SecurityServer.generatePaymentToken(paymentRequest.msisdn, _: ProductType, paymentRequest.tempCode)
      PaymentResult(paymentRequest, paymentService.withdrawPayment(paymentRequest, token))
  }
  ...
  val paymentDomains: List[PaymentDomain] = serverPool.sortBy(_.geographicalPriority) map createPaymentDomain toList

  val paymentDomain: BusinessDomain[PaymentRequest, PaymentResult] = chainDomains(paymentDomains, defaultPaymentDomain)

  val postPaymentServiceChain: PostPaymentDomain = postPaymentDomain

  val paymentFlow = paymentDomain andThen postPaymentServiceChain

  val result: immutable.Seq[PaymentStatus] = payments map paymentFlow
